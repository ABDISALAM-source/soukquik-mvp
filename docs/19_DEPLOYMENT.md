...
- **WebSocket présence (Phase 8)** : le serveur WS partage le serveur HTTP Express (même port 4000, chemin `/ws/presence`), donc il passe par le même tunnel Cloudflare — vérifié en `wss://` en production, aucun port ni service supplémentaire à exposer. Cloudflare Tunnel gère nativement l'upgrade WebSocket. Derrière un reverse proxy Nginx classique, penser à `proxy_set_header Upgrade $http_upgrade;` + `proxy_set_header Connection "upgrade";` sur ce chemin.
- **Connexion Google (OAuth)** : `POST /auth/google` vérifie le token côté serveur via l'API userinfo publique de Google (aucun secret serveur requis). Côté mobile, renseigner `webClientId`/`iosClientId`/`androidClientId` réels dans `Google.useAuthRequest` (AuthScreen) — placeholders actuels pour le web/iOS.
